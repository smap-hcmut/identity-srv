...
var JWTKeyWhere = struct {
	...
	Kid:        whereHelperstring{field: "\"identity\".\"jwt_keys\".\"kid\""},
	PrivateKey: whereHelperstring{field: "\"identity\".\"jwt_keys\".\"private_key\""},
	PublicKey:  whereHelperstring{field: "\"identity\".\"jwt_keys\".\"public_key\""},
	Status:     whereHelperstring{field: "\"identity\".\"jwt_keys\".\"status\""},
	CreatedAt:  whereHelpertime_Time{field: "\"identity\".\"jwt_keys\".\"created_at\""},
	ExpiresAt:  whereHelpernull_Time{field: "\"identity\".\"jwt_keys\".\"expires_at\""},
	RetiredAt:  whereHelpernull_Time{field: "\"identity\".\"jwt_keys\".\"retired_at\""},
}
...
func JWTKeys(mods ...qm.QueryMod) jwtKeyQuery {
	mods = append(mods, qm.From("\"identity\".\"jwt_keys\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"identity\".\"jwt_keys\".*"})
	}
...
func FindJWTKey(ctx context.Context, exec boil.ContextExecutor, kid string, selectCols ...string) (*JWTKey, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"identity\".\"jwt_keys\" where \"kid\"=$1", sel,
	)
...
func (o *JWTKey) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"identity\".\"jwt_keys\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"identity\".\"jwt_keys\" %sDEFAULT VALUES%s"
		}
...
func (o *JWTKey) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"identity\".\"jwt_keys\" SET %s WHERE %s",
...
func (o JWTKeySlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"identity\".\"jwt_keys\" SET %s WHERE %s",
...
func (o *JWTKey) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"identity\".\"jwt_keys\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *JWTKey) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"identity\".\"jwt_keys\" WHERE \"kid\"=$1"
...
func (o JWTKeySlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"identity\".\"jwt_keys\" WHERE " +
...
func (o *JWTKeySlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"identity\".\"jwt_keys\".* FROM \"identity\".\"jwt_keys\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, jwtKeyPrimaryKeyColumns, len(*o))
...
func JWTKeyExists(ctx context.Context, exec boil.ContextExecutor, kid string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"identity\".\"jwt_keys\" where \"kid\"=$1 limit 1)"
